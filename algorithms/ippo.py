...
from networks import PPOCritic
...
class IPPO:
    def __init__(self, args):
        ...
        self.model_dir = args.model_dir + '/' + args.alg + '/' + "trial_5" 
        ...
        for _ in range(self.args.ppo_n_epochs):
            ...
            for transition_idx in reversed(range(max_episode_len)):
                ...
                advantages[:, transition_idx] = deltas[:,
                                                transition_idx] + self.args.gamma * self.args.gae_lambda * prev_advantage * (
                                                            1 - terminated[:, transition_idx]) * mask[:, transition_idx]
            ...
            surr2 = torch.clamp(ratios, 1.0 - self.args.clip_coef, 1.0 + self.args.clip_coef) * advantages

            entropy = dist.entropy()
            entropy[mask == 0] = 0.0

            policy_loss = torch.min(surr1, surr2) + self.args.ent_coef * entropy

            policy_loss = - (policy_loss * mask).sum() / mask.sum()

            error_clip = torch.clamp(values - old_values.detach(), -self.args.clip_coef,
                                     self.args.clip_coef) + old_values.detach() - returns
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.ac_parameters, self.args.max_grad_norm)
